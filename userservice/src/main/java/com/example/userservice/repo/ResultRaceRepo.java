...
public interface ResultRaceRepo extends JpaRepository<ResultRace, Integer>{
    ...
    @Query(value = "select * from result_race where id= :id", nativeQuery = true)
    Collection<ResultRace> findResultRaceByIdDriver(@Param("id") Integer id);
    ...
    int updateDriver(@Param("id") Integer id, @Param("points") Integer points, @Param("dnf") boolean dnf, @Param("dns") boolean dns, @Param("dnq") boolean dnq, @Param("vr_time") LocalTime vr_time, @Param("date_updated") Date date_updated);
}
